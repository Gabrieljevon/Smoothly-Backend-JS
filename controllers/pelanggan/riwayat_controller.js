...
exports.showRiwayat = async (req, res) => {
    ...
    let vShowRiwayat = [id_pelanggan];

    if (status_pesanan !== undefined && status_pesanan !== null && status_pesanan !== '') {
        queryStatusPesanan = 'AND ps.status_pemesanan = ?';
        vShowRiwayat.push(status_pesanan);
    }
    ...
    connection.query(qShowRiwayat, vShowRiwayat, (error, rows) => {
        if (error) {
            console.log(error);
